...
def extract_features(feature_types, datasets, outdir_parent):
  for feature_type in feature_types:
    outdir = '{}/{}'.format(outdir_parent, feature_type)
...
def scale_dataset(train_X, test_pos_X, test_neg_X):
  ...
  return train_X, test_pos_X, test_neg_X
...
def create_train_test_dfs(feature_type, outdir, data_list):
  ...
  #train_test split
  # train_X, test_X, train_y, test_y = train_test_split(train_df, test_df)
  ...
  train_X, test_pos_X, test_neg_X = scale_dataset(train_X, test_pos_X, test_neg_X)

  return train_X, test_pos_X, test_neg_X, train_y, test_pos_Y,  test_neg_Y
...
def create__pos_neg_test_dfs(feature_type, outdir, data_list):
...
datasets = ['TR_neg_SPIDER', 'TR_pos_SPIDER', 'TS_neg_SPIDER', 'TS_pos_SPIDER']
outdir_parent = ''
...
print("\nClassfier using AAC feature type")
feature_type = 'AAC'
AAC_train_X, AAC_test_X, AAC_train_y, AAC_test_y = create_train_test_dfs(feature_type, outdir_parent, datasets)
...
print("\nClassfier using GDPC feature type")
feature_type = 'GDPC'
GDPC_train_X, GDPC_test_X, GDPC_train_y, GDPC_test_y = create_train_test_dfs(feature_type, outdir_parent, datasets)
...
print("\nClassfier using DPC feature type")
feature_type = 'DPC'
DPC_train_X, DPC_test_X, DPC_train_y, DPC_test_y = create_train_test_dfs(feature_type, outdir_parent, datasets)
...
print("\nClassfier using PAAC feature type")
feature_type = 'PAAC'
PAAC_train_X, PAAC_test_X, PAAC_train_y, PAAC_test_y = create_train_test_dfs(feature_type, outdir_parent, datasets)
...
print("\nVoting classifier")
model_list = [(AAC_test_X, xgb_cl_AAC),(DPC_test_X, xgb_cl_DPC)]
...
with open('predictions_pos.txt', 'w') as file:
    for prediction in ensemble_preds:
